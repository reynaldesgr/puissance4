...
public class Grid {
    ...
    /*
    public int comptAlignToken(int length, Player.COLOR color){
        int nb = 0;
        for(int l = 1; l <= Grid.MAX_LINE; l++){
            for(int c = 1; c <= Grid.MAX_COLUMN; c++){
                if(this.checkAlignementToken(length, color, l, c, 0, 1))   nb++;
                if(this.checkAlignementToken(length, color, l, c, 1, 1))   nb++;
                if(this.checkAlignementToken(length, color, l, c, 1, 0))   nb++;
                if(this.checkAlignementToken(length, color, l, c, 1, -1))  nb++;
                if(this.checkAlignementToken(length, color, l, c, 0, -1))  nb++;
                if(this.checkAlignementToken(length, color, l, c, -1, -1)) nb++;
                if(this.checkAlignementToken(length, color, l, c, -1, 0))  nb++;
