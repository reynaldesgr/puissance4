...
public class AlphaBetaIA 
{
    ...
    public int bestColumnToPlay(Grid grid, Player p)
    {
        ...
        for(int m : columnToPlay)
        {
            ...
            if(val > alpha && !grid.isColumnFull(m - 1))
            {
                action  = m;
                alpha   = val;
            }
    ...
    /* Consiste à minimiser la perte maximum. L'élègage alpha-beta accélère la routine */
    ...
    private int alphaBeta(Grid grid, Player j, int alpha, int beta, boolean maxState, int depth)
    {
        ...
        if(depth == 0)
        { /* Maximum de profondeur atteint. On calcule le score obtenu. */
