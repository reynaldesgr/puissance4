...
public class Grid {
    ...
    public static       int             last_token_pos_l    = 0,  // Dernière position de la colonne du token joué
                                        last_token_pos_c    = 0; // Dernière position de la ligne du token joué

    public static final Token           EMPTY               = null;
    private static      FileGridSaver   fSaver              = new FileGridSaver();

    private             Token[][]       grid;
    private             int[]           next_free_case;  // Tableau de référence sur la prochaine case libre de chaque colonne.
    ...
    /* Initialise la référence de la prochaine case libre pour chaque colonne de la grille.*/
    public void initializeIndex()
    {
        this.next_free_case = new int[MAX_COLUMN];
        for(int c = 0; c < Grid.MAX_COLUMN; c++)
        {
            for(int l = MAX_LINE - 1; l > 0; l--)
            {
    ...
    public Grid()
    { 
        this.grid = new Token[MAX_LINE][MAX_COLUMN];
        initializeIndex();
    }
    ...
    public void loadGrid(String path, Player J1, Player J2)
    { 
        this.setGrid(fSaver.load(path, J1, J2)); 
    }

    /* Sauvegarde la grille du jeu en cours */
    public void saveGrid(String path, Player J1, Player J2)
    { 
        fSaver.save(path, this, J1, J2);
    }
    ...
    {
        return next_free_case[column] == -1 ? true : false;
    }
    ...
    /* Récupère la copie d'une grille. 
    /* -> (sert surtout pour l'IA pour simuler les coups et l'arbre des possibles.) */
